      ...
      <section className="relative w-full overflow-hidden bg-slate-900 text-white">
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col items-start px-6 py-20 md:px-12 md:py-32 lg:py-40">
          ...
          <div className="flex flex-col gap-4 sm:flex-row w-full sm:w-auto">
            <Button size="lg" className="h-14 rounded-full bg-white px-8 text-base font-bold text-slate-900 shadow-xl transition-all hover:scale-105 hover:bg-slate-100" asChild>
            ...
            <Button size="lg" variant="outline" className="h-14 rounded-full border-white/30 bg-white/5 px-8 text-base font-bold text-white shadow-xl backdrop-blur-md transition-all hover:bg-white/10 hover:text-white" asChild>
              <Link href="/about">
        ...
        <div className="mb-10 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 text-lg text-muted-foreground">
              {locale === "th" ? "สินค้าที่ได้รับความนิยมสูงสุดในขณะนี้" : "Our most popular items right now"}
            </p>
          </div>
          <Button variant="ghost" className="hidden md:flex font-semibold" asChild>
            <Link href="/categories">
              {locale === "th" ? "ดูทั้งหมด" : "View All"} <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </div>
